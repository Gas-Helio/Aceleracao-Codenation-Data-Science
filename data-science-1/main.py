...
def q1():
    ...
    d = dataframe.describe().loc[['25%', '50%', '75%']]
    sub = lambda x: x['normal'] - x['binomial']
    return tuple(map(lambda x: round(x, 3), (sub(d.iloc[0]), sub(d.iloc[1]), sub(d.iloc[2]))))
...
def q5():
    ...
    return tuple(
        map(lambda x: round(x, 3), 
            false_pulsar_mean_profile_standardized.quantile([.25, .5, .75]).values - sct.norm.ppf([.25, .5, .75])
           )
    )
